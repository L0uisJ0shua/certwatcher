...
install:
	...
	@echo "[INFO] Moving the binary to ~/bin/certwatcher"
	@if [ -f "~/bin/certwatcher" ]; then \
		read -p "[WARN] A binary already exists at ~/bin/certwatcher. Do you want to delete it? (y/n) " answer; \
		rm -f "~/bin/certwatcher"; \
	fi
	@cp cmd/certwatcher/certwatcher ~/bin/certwatcher
	@echo "[INFO] Creating hidden directory .certwatcher-templates at the user home directory"
	@mkdir -p ~/certwatcher-templates
	@echo "[INFO] Certwatcher binary successfully installed at ~/bin/certwatcher."
