...
func TestRequestsPost(t *testing.T) {
	...
	// Create requests with multiple paths
	// Criar uma requisição POST com corpo
	...
	// Definir a URL alvo
	url := "https://ci56dk6omrrgj17hc9qgmgdeqry7o7c3z.oast.live"

	respPost, statusPost, sizesPost, err := http.Requests(url, reqPost)
	if err != nil {
		gologger.Fatal().Msgf("Erro na requisição POST: %s", err)
	}

	fmt.Printf("Resposta POST: %v\n", respPost)
	fmt.Printf("Status POST: %v\n", statusPost)
	fmt.Printf("Tamanhos POST: %v\n", sizesPost)
}
